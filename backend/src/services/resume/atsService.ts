...
import { AnalysisResult, SavedResumeResult } from "../../types/aiResumeType";
...
const computeAnalysis = async (
  ...
  return { score, keywords, sections, suggestions, feedback, aiResult };
};
...
export const analyzeAndSaveResume = async (
  ...
  const { score, keywords, sections, suggestions, feedback, aiResult } =
    await computeAnalysis(resumeText, jobDescription);
...
export const recompareById = async (id: string) => {
